...
def process(model:FLST1Model,
	...
	for d in tqdm(data):
		...
		y.append(np.argmax(r['membership_values']))#alpha-cut (0.5)

	y = np.asarray(y)

	print(f'[Fuzzy Logic System - T1][{mode}] results:')
	_c, _l = np.unique(y, return_counts=True)
	for i, c in zip(_c,_l):
		print (f'\tc_{i}: {c}')

	
	result_file = f'results_fuzzyt1_{mode}_{obs_len}s_{filter_name}.csv'
	print (f'[Fuzzy Logic System - T1][{mode}] saving results ({result_file})...')
...
if __name__ == '__main__':
	...
	print ('[Fuzzy Logic System - T1] running....') 
	...
	assert os.path.exists(args.data_dir),\
		f'[Fuzzy Logic System - T1][main][ERROR] data_dir not found!({args.data_dir})'
	...
	assert os.path.exists(data_file),\
		f'[Fuzzy Logic System - T1][main][ERROR] data_file not found!({data_file})'


	assert os.path.exists(args.rules_dir),\
		f'[Fuzzy Logic System - T1][main][ERROR] rules_dir not found!({args.rules_dir})'


	print ('[Fuzzy Logic System - T1] loading dataset....')
	# [mean_v, mean_acc, mean_deac, std_jy] (m, 4)
	data = np.load(os.path.join(args.data_dir,data_file))


	print('[Fuzzy Logic System - T1] bulding Fuzzy Logic System - T1 model...')
	fuzz = FLST1Model(rules_path=args.rules_dir, expert_mode=args.expert_mode)
	
	print('[Fuzzy Logic System - T1] processing data...')
